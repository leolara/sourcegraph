...
func (s *Server) cleanupRepos() {
	...
	maybeReclone := func(dir GitDir) (done bool, err error) {
		...
		repo := s.name(dir)
		s.Logger.Info("re-cloning expired repo", log.String("repo", string(repo)), log.Time("cloned", recloneTime), log.String("reason", reason))
		...
		if err := setRecloneTime(dir, recloneTime.Add(time.Since(recloneTime)/2)); err != nil {
			s.Logger.Warn("setting backed off re-clone time failed",
	...
	err := bestEffortWalk(s.ReposDir, func(dir string, fi fs.FileInfo) error {
		...
		for _, cfn := range cleanups {
			...
			if err != nil {
				s.Logger.Error("error running cleanup command", log.String("name", cfn.Name), log.String("repo", string(gitDir)), log.Error(err))
			}
	...
	if err != nil {
		s.Logger.Error("cleanup: error iterating over repositories", log.Error(err))
	}

	if b, err := json.Marshal(stats); err != nil {
		s.Logger.Error("cleanup: failed to marshal periodic stats", log.Error(err))
	} else if err = os.WriteFile(filepath.Join(s.ReposDir, reposStatsName), b, 0666); err != nil {
		s.Logger.Error("cleanup: failed to write periodic stats", log.Error(err))
	}
	...
	s.setRepoSizesOnce.Do(func() {
		...
		if err != nil {
			s.Logger.Error("cleanup: setting repo sizes", log.Error(err))
		}
	})
	...
	if err != nil {
		s.Logger.Error("cleanup: ensuring free disk space", log.Error(err))
	}
	if err := s.freeUpSpace(b); err != nil {
		s.Logger.Error("cleanup: error freeing up space", log.Error(err))
	}
}
...
func (s *Server) freeUpSpace(howManyBytesToFree int64) error {
	...
	for _, d := range gitDirs {
		...
		s.Logger.Warn("cleanup: removed least recently used repo",
